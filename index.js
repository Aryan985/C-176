...
function displayStory(){
    var num = stories[Math.floor(Math.random()*3)]
    $("#story_title").html(num.title)
    $("#input_fields").empty();
    for (let i = 0; i < num.inputs; i++) {
    ...
    for (let i = 0; i < num.words.length; i++) {
        let html = `<button class="word_bank_button">${num.words[i]}</button>`
        $("#bank_words").append(html)
    }
    $("#story_text").html(num.story)
}

$(function(){
    $(".input_field").keyup(function(){
        var id = $(this).attr("id")
        var inputnum = id.split("_")[1]
        $(".rep_input").eq(inputnum).html($(this).val())
    })
